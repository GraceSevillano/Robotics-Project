...
class NiryoConnection:
    def __init__(self):
        ...
        self.sub_niryo = rospy.Suscriber("/niryo_con",UInt8, self.niryoCallBack,queue_size=1)
        self.pub_niryo = rospy.Publisher("/niryo_con",UInt8, queue_size=10)
        self.counter = 0

    def nyrioCallBack(self,msgs):
        self.counter += 1
        if self.counter <= 1:
            if msg.data ==1:
                robot = NyrioRobot("192.168.0.150")
                ...
                except NyrioRosWrapperException as e:
